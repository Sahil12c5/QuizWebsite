        ...
        if (pathInfo != null) {
            ...
            if (splits.length >= 2) {
                try {
                    ...
                    int high = userScoreDAO.getHighestScore(userId);
                    int low = userScoreDAO.getLowestScore(userId);
                    List<UserScore> history = userScoreDAO.getUserHistory(userId);

                    profileData.put("totalQuizzesTaken", totalTaken);
                    profileData.put("averageScore", Math.round(avgScore * 100.0) / 100.0);
                    profileData.put("highestScore", high);
                    profileData.put("lowestScore", low);
