...
public class UserScoreDAO {
    ...
    public boolean saveScore(int userId, int quizId, int score, int totalQuestions, int timeSpent) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    us.setTimeSpent(rs.getInt("time_spent"));
    ...
    public int getHighestScore(int userId) {
        String sql = "SELECT MAX(score) FROM user_scores WHERE user_id = ?";
        try (Connection conn = DatabaseConfig.getConnection();
                PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, userId);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    public int getLowestScore(int userId) {
        String sql = "SELECT MIN(score) FROM user_scores WHERE user_id = ?";
        try (Connection conn = DatabaseConfig.getConnection();
                PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, userId);
            try (ResultSet rs = pstmt.executeQuery()) {
